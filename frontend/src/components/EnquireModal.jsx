...
export default function EnquireModal({ vehicle, isOpen, onClose }) {
  ...
  const whatsappMessage = encodeURIComponent(
    `Hello RPM Vault! I want to enquire about ${vehicle.name} (${vehicle.formattedPrice}). Link: ${window.location.href}`
  );
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
        <motion.div
          ...
          className="bg-[#141418] border border-white/15 rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl relative overflow-hidden"
        >
          {/* Close button */}
          <button
            onClick={onClose}
